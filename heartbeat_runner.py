...
def step7_invoke_claude(
    ...
    _use_openai = os.environ.get("CLAUDE_CODE_USE_OPENAI") == "1"
    ...
    try:
        import urllib.request as _urllib_req, urllib.error as _urllib_err, json as _json_p, logging as _log_p
        ...
        # POLÍTICA: OpenRouter free — openclaude não precisa de Anthropic auth.
        _openrouter_key = os.environ.get("OPENROUTER_API_KEY", "")

        # Selecionar modelo free disponível com pre-flight rápido
        _free_models = [
            "nvidia/nemotron-3-super-120b-a12b:free",
            "google/gemma-4-31b-it:free",
            "google/gemma-4-26b-a4b-it:free",
        ]
        _model = _free_models[0]  # default
        for _m in _free_models:
            try:
                _chk = _check_openai_compat("https://openrouter.ai/api/v1", _openrouter_key, _m)
                if _chk:
                    _model = _m
                    break
            except Exception:
        ...
        # Env limpo: só OpenAI compat, sem Anthropic (openclaude não exige)
        _selected_env = dict(os.environ)
        _selected_env["IS_SANDBOX"] = "1"
        _selected_env["CLAUDE_CODE_USE_OPENAI"] = "1"
        _selected_env["OPENAI_BASE_URL"] = "https://openrouter.ai/api/v1"
        _selected_env["OPENAI_API_KEY"] = _openrouter_key
        _selected_env["OPENAI_MODEL"] = _model
        _selected_env["CLAUDE_CODE_PROVIDER_PROFILE_ENV_APPLIED"] = "1"
        # Remover vars Anthropic — openclaude funciona sem elas no modo OpenAI compat
        _selected_env.pop("ANTHROPIC_API_KEY", None)
        _selected_env.pop("ANTHROPIC_BASE_URL", None)
        _selected_env.pop("OPENAI_API_KEY_DIRECT", None)
        _selected_cmd = cmd
        print(f"[provider] OpenRouter-free/{_model.split('/')[1]}", flush=True)
        ...
        try:
            ...
            if proc.returncode != 0:
                # Exit code 1 sem erro real = agente sem trabalho (skip normal)
                real_errors = [l for l in (stderr or "").split("\n")
                               if l.strip() and "[context] Warning" not in l
                               and "not in integration model metadata" not in l]
                if proc.returncode == 1 and not real_errors:
                    status = "success"  # "sem trabalho" conta como sucesso
                    error = None
                else:
                    status = "fail"
